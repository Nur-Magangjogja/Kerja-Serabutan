            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between min-h-[40px] text-white">
                    ...
                    <div class="text-center flex-1 min-w-0 px-2">
                        <h1 class="text-base font-bold truncate">Tarik Saldo Dompet</h1>
                        <p class="text-xs text-white/90 truncate mt-0.5">Kirim dana ke rekening bank / e-wallet</p>
                    </div>

                    <div class="w-10 flex items-center justify-end">
                        <a href="{{ route('customer.withdraw.history') }}" wire:navigate class="p-2 hover:bg-white/20 rounded-xl transition-colors duration-200 text-xs font-bold flex items-center justify-center text-white" title="Riwayat Penarikan" aria-label="Riwayat Penarikan">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-5 pt-5 pb-24 space-y-4">
            ...
            <div class="bg-gradient-to-br from-[#0098e7] via-[#0077cc] to-[#005da6] rounded-2xl p-5 text-white shadow-md flex items-center justify-between">
                <div class="min-w-0 pr-2">
                    ...
                    <span class="text-[10px] text-white/70 block mt-0.5">Dapat ditarik ke rekening kapan saja</span>
                </div>
            ...
            <form wire:submit="submit" class="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xs space-y-4">
                <div>
                    ...
                    <div x-data="{
                        ...
                        <input type="text"
                            ...
                            class="w-full pl-12 pr-4 py-3 text-sm font-bold border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:ring-2 focus:ring-primary-500">
                    </div>
                    @error('amount') <span class="text-rose-500 text-xs mt-1 block font-medium">{{ $message }}</span> @enderror
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Batas penarikan min: Rp {{ number_format($minAmount, 0, ',', '.') }} (Kelipatan Rp 100)</p>
                </div>
                ...
                <div x-data="{
                    ...
                    >
                        ...
                        <div class="shrink-0 text-gray-400 dark:text-gray-400 transition-transform duration-200" :class="open ? 'rotate-180 text-primary-600 dark:text-sky-400' : ''">
                        ...
                        <div class="p-2.5 border-b border-gray-200/80 dark:border-gray-600/80 bg-white/60 dark:bg-gray-800/60">
                            <div class="relative">
                                ...
                                <input
                                    ...
                                    class="w-full pl-8 pr-7 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-xs text-gray-900 dark:text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-primary-500"
                                />
                                <button
                                    type="button"
                                    ...
                                    <div class="space-y-0.5 mt-0.5">
                                        <template x-for="b in getBanksByCategory(cat)" :key="b.code">
                                            <button
                                                ...
                                                :class="String(bankCode).toUpperCase() === b.code.toUpperCase() ? 'bg-primary-50 dark:bg-primary-950/60 text-primary-700 dark:text-sky-300 font-semibold ring-1 ring-primary-300 dark:ring-primary-700' : 'hover:bg-white dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200'"
                                            >
                                                <div class="flex items-center gap-2.5 min-w-0 flex-1 pr-2">
                                                ...
                                                <div class="shrink-0 flex items-center">
                                                    <span x-show="String(bankCode).toUpperCase() === b.code.toUpperCase()" class="w-5 h-5 rounded-full bg-primary-600 text-white flex items-center justify-center text-[10px] font-black">
                                                        ✓
                                                    </span>
                                                </div>
                                            </button>
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="submit" class="w-full py-3 bg-gradient-to-r from-[#0098e7] to-[#0077cc] hover:opacity-95 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed disabled:pointer-events-none">
                    ...
                    <span wire:loading.remove wire:target="submit">Ajukan Penarikan Dana Sekarang</span>
                    <span wire:loading wire:target="submit">Memproses Penarikan...</span>
                </button>
            </form>
        </div>
    </div>
