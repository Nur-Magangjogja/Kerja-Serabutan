<div class="space-y-4 max-w-5xl mx-auto w-full" wire:poll.10s.visible>
    ...
    {{-- Header Navigation --}}
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-xs">
        <div class="flex items-start sm:items-center gap-3 min-w-0">
            <a href="{{ route($routePrefix . 'partners.reports.show', $report) }}"
                class="p-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-xl text-xs font-semibold transition flex items-center gap-1 shrink-0">
            ...
            <div class="min-w-0">
                ...
                <p class="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400">Jalur obrolan 1-on-1 Admin dengan Pelapor & Mitra</p>
            </div>
        </div>
    ...
    <div class="bg-purple-50/70 dark:bg-purple-950/40 p-3.5 rounded-2xl border border-purple-200 dark:border-purple-800/60 shadow-xs flex items-start sm:items-center justify-between gap-3 flex-wrap">
        <div class="min-w-0 flex-1">
            ...
            <p class="text-xs font-semibold text-purple-950 dark:text-purple-100 mt-0.5 break-words [overflow-wrap:anywhere]">"{{ $report->message }}"</p>
        </div>
    ...
    <div class="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-2 overflow-x-auto scrollbar-none">
        <button type="button" wire:click="selectTab('customer')"
            ...
            <span>Chat Pelapor: {{ $customer?->name ?? 'Customer' }}</span>
            @if($unreadCustomer > 0)
        ...
        <button type="button" wire:click="selectTab('mitra')"
            class="flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs transition cursor-pointer shrink-0 whitespace-nowrap {{ $activeTab === 'mitra' ? 'bg-emerald-600 text-white shadow-xs font-bold' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700 font-medium' }}">
            <span class="text-sm">🛵</span>
            <span>Chat Mitra: {{ $mitra?->name ?? 'Mitra' }}</span>
            @if($unreadMitra > 0)
                <span class="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-rose-500 text-white animate-pulse">
                    {{ $unreadMitra }} Baru
                </span>
            ...
            @else
                @foreach($messages as $msg)
                    ...
                    <div class="flex flex-col {{ $isMe ? 'items-end' : 'items-start' }}">
                        ...
                        <div class="max-w-[88%] sm:max-w-[75%] rounded-2xl p-3 text-xs shadow-xs {{ $isMe ? 'bg-primary-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white rounded-bl-none' }}">
                            @if($msg->photo)
