<x-mitra-layout>
    ...
    @php
        ...
        $activeHelps = \App\Models\Help::where('mitra_id', $user->id)->active()->count();
    @endphp
        ...
        <div class="px-5 pt-4 pb-5 relative overflow-hidden bg-gradient-to-br from-[#0098e7] via-[#0077cc] to-[#0060b0] rounded-b-2xl shadow-sm text-white">
            ...
            <div class="relative z-10">
                <div class="flex items-center justify-between text-white">
                    <a href="{{ route('mitra.profile') }}" aria-label="Kembali" class="p-2 hover:bg-white/20 rounded-xl transition cursor-pointer flex-shrink-0">
                    ...
                    <div class="text-center flex-1 min-w-0 px-2">
                        <h1 class="text-base font-bold truncate">Pengaturan Mitra</h1>
                        <p class="text-xs text-white/90 truncate mt-0.5">Kelola preferensi dan keamanan akun Anda</p>
                    </div>

                    <a href="{{ route('mitra.notifications.index') }}" class="p-2 hover:bg-white/20 rounded-xl transition cursor-pointer relative flex-shrink-0" title="Notifikasi">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($lastRejected)
                        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/60 rounded-2xl p-4 space-y-2">
                            ...
                            <p class="text-[11px] text-gray-500 dark:text-gray-400">
                                Silakan lakukan penarikan sisa saldo (withdraw) terlebih dahulu, lalu Anda dapat mengajukan permohonan baru di bawah.
                            </p>
                        </div>
                    @endif
    ...
    <div id="deletionModal"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4">
            ...
            <div class="text-center">
                <h2 class="text-lg font-bold text-gray-900 dark:text-white">Permintaan Hapus Akun Mitra</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Permintaan penutupan akun Mitra akan ditinjau oleh Superadmin untuk memverifikasi saldo dan pekerjaan aktif.
                </p>
            </div>
            ...
            @if($userBalance > 0)
                <div class="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-xl p-3.5 text-xs text-amber-800 dark:text-amber-200 space-y-1">
                    <div class="font-bold flex items-center gap-1.5">
                        ...
                        Sisa Saldo Pendapatan: Rp {{ number_format($userBalance, 0, ',', '.') }}
                    </div>
                    <p class="text-[11px] leading-relaxed opacity-90">
                        Harap ajukan penarikan saldo (withdraw) terlebih dahulu. Akun dengan sisa saldo aktif dapat ditolak oleh Superadmin sampai penarikan berhasil.
                    </p>
                </div>
            @endif
            ...
            @if($activeHelps > 0)
                <div class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl p-3 text-xs text-red-800 dark:text-red-200">
                    <strong>Perhatian:</strong> Anda masih memiliki {{ $activeHelps }} tugas bantuan yang sedang aktif dikerjakan. Harap selesaikan pekerjaan terlebih dahulu.
                </div>
            @endif
            ...
            <form action="{{ route('profile.deletion.request') }}" method="POST" class="space-y-3.5">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">
                        Alasan Pengajuan Hapus Akun <span class="text-red-500">*</span>
                    </label>
                    <textarea name="reason" rows="3" required
                        placeholder="Contoh: Sudah tidak aktif menerima pesanan bantuan, pindah domisili, dll."
