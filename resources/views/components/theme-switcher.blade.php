...
class="inline-flex p-1.5 bg-gray-200 dark:bg-gray-800 rounded-2xl gap-1.5 border border-gray-300 dark:border-gray-700 transition-colors duration-200 shadow-inner">
    ...
    <!-- Light Mode Button -->
    <button type="button"
        @click="currentTheme = 'light'; window.setTheme('light')"
        ...
        :class="currentTheme === 'light' 
            ? 'bg-amber-500 text-white shadow-md ring-2 ring-amber-400/50 scale-105' 
            : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700/50'">
        <svg class="w-4 h-4" :class="currentTheme === 'light' ? 'text-white' : 'text-amber-500'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Dark Mode Button -->
    <button type="button"
        @click="currentTheme = 'dark'; window.setTheme('dark')"
        class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 cursor-pointer"
        :class="currentTheme === 'dark' 
            ? 'bg-indigo-600 text-white shadow-md ring-2 ring-indigo-400/50 scale-105' 
            : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700/50'">
        <svg class="w-4 h-4" :class="currentTheme === 'dark' ? 'text-white' : 'text-indigo-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- System Button -->
    <button type="button"
        @click="currentTheme = 'system'; window.setTheme('system')"
        class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 cursor-pointer"
