...
@php
    ...
    try {
        if (auth()->check()) {
            ...
            // 1. Unread chat from mitras & system
            $unreadMitra = \App\Models\Chat::where('customer_id', $customerId)
                ->whereIn('sender_type', ['mitra', 'system'])
                ->whereNull('read_at')
                ->count();
                
            // 2. Unread messages from admin report
            $customerReports = \App\Models\PartnerReport::where('reporter_id', $customerId)
                ->orWhere('reported_user_id', $customerId)
                ->orWhereHas('reportedHelp', function($q) use ($customerId) {
                    $q->where('user_id', $customerId);
                })
                ->pluck('id');
                
            $unreadAdmin = \App\Models\PartnerReportMessage::whereIn('partner_report_id', $customerReports)
                ->where('recipient_type', 'customer')
                ->where('is_read', false)
                ->count();
                
            $unread = $unreadMitra + $unreadAdmin;
        }
    } catch (\Exception $e) {
        $unread = 0;
    }
