...
<body class="antialiased bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100" 
      ...
      x-init="$watch('sidebarOpenMobile', val => { if(val) { $nextTick(() => { $refs.adminSidebarNav?.focus(); }); } })"
            ...
            <nav x-ref="adminSidebarNav"
                ...
                <a href="{{ route('admin.withdraws.index') }}" wire:navigate
                    ...
                    @if($pendingWithdrawsCount > 0)
                        <span class="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10px] font-extrabold {{ request()->routeIs('admin.withdraws.*') ? 'bg-white text-rose-600 shadow-2xs' : 'bg-rose-500 text-white shadow-xs' }} ml-2 shrink-0 animate-pulse" title="{{ $pendingWithdrawsCount }} Permintaan Withdraw Menunggu Proses">
                ...
                <a href="{{ route('admin.topup.approvals') }}" wire:navigate
                    ...
                    @if($pendingTopupsCount > 0)
                        <span class="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-[10px] font-extrabold {{ request()->routeIs('admin.topup.approvals*') ? 'bg-white text-amber-600 shadow-2xs' : 'bg-amber-500 text-white shadow-xs' }} ml-2 shrink-0 animate-pulse" title="{{ $pendingTopupsCount }} Permintaan Top-Up Menunggu Approval">
